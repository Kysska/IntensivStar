...
class UpcomingMovieRepositoryImpl(private val movieApiInterface: MovieApiInterface) {

    fun getUpcomingMovies(callback: (CustomResult<List<MovieCard>>) -> Unit) {
        callback(CustomResult.Loading)

        movieApiInterface.getUpcomingMovies().enqueue(object : Callback<MovieListResponse> {
